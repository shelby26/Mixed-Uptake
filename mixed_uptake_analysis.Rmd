...
# remove plate 1 samples for now...
polyp <- droplevels(polyp[which(polyp$Plate!="1"), ])
...
```{r samdat}
sam <- read.csv("qPCRdata.csv")  # sample data is called "qPCRdata.csv"
...
df$mixed <- ifelse(nclades[as.character(df$Sample), "x"] > 1, TRUE, FALSE)
...
deWit <- function(s1, s2, time, data) {
  ...
  plot(Q ~ as.numeric(Sym_ID), data=light, xaxt="n", xlab="", main="Light", ylim=c(0, max),
       col=c("blue", "red", "green")[match(Target, c("A","B","D"))])
  ...
  for (s in levels(factor(light$Target))) {
    lines(Q ~ as.numeric(Sym_ID), lty=2, col=c("blue", "red", "green")[match(Target, c("A","B","D"))],
          data=subset(light, Target==s & nchar(as.character(Sym_ID))==1))
    lines(Q ~ as.numeric(Sym_ID), lty=1, col=c("blue", "red", "green")[match(Target, c("A","B","D"))],
  ...
  dark$`Q+se` <- 10^(dark$lsmean + light$SE)
  dark$`Q-se` <- 10^(dark$lsmean - light$SE)
